...
{
    // TODO: Investigate whether or not we need to set self.waitingOnCaller to NO if this method is called with zero bytes, or if it returns zero bytes.
    @synchronized(self)
    {
    ...
    {
        dataCopied = [self.blobUploadHelper write:buffer length:length completionHandler:^{
            [self fireStreamEvent];
        }];        
    }
    else
...
-(void)close
{
    ...
    [self.blobUploadHelper closeWithCompletionHandler:^{
...
{
    return self.blobUploadHelper.hasSpaceAvailable;
}
